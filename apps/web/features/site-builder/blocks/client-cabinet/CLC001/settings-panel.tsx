...
export function updateCabinetStyle(ctx: CrmPanelCtx, patch: StyleMap) {
  ctx.updateBlock(ctx.block.id, (prev) => {
    ...
    return { ...prev, type: "clientCabinet", data: { ...data, clientView: "cabinet", style: { ...style, ...patch } } };
  });
}
...
function BackgroundField({
  ...
  return (
    <TildaBackgroundColorField
      ...
      mode={mode}
      secondValue={to}
      angle={readNumber(style, angleKey, 135)}
      radialStopA={readNumber(style, stopAKey, 0)}
      radialStopB={readNumber(style, stopBKey, 100)}
      placeholder={fallback}
      onModeChange={(value) => updateCabinetStyle(ctx, { [modeKey]: value })}
      onSecondChange={(value) => updateCabinetStyle(ctx, { [toKey]: value })}
      onAngleChange={(value) => updateCabinetStyle(ctx, { [angleKey]: value })}
      onRadialStopAChange={(value) => updateCabinetStyle(ctx, { [stopAKey]: value })}
      onRadialStopBChange={(value) => updateCabinetStyle(ctx, { [stopBKey]: value })}
      onChange={(value) =>
        updateCabinetStyle(ctx, {
          [legacyKey]: value,
          [fromKey]: value,
...
function drawerButton(id: string, label: string, ctx: CrmPanelCtx) {
  return (
    <button
      ...
      style={{
        borderColor: ctx.activePanelSectionId === id ? "#2F8EEF" : ctx.panelTheme.border,
...
export function ClientCabinetSettingsPanel(ctx: CrmPanelCtx) {
  const [darkOpen, setDarkOpen] = useState(false);
  const style = readStyle(ctx);
  const columns = Math.max(1, Math.min(MAX_BLOCK_COLUMNS, readNumber(style, "blockWidthColumns", 8)));
    ...
    <div className="space-y-5 px-1 pb-8 pt-1" onClick={(event) => event.stopPropagation()}>
      <div className="relative">
        ...
        {ctx.coverWidthModalOpen ? (
          <div ref={ctx.getCoverWidthPopoverRef()} className="absolute inset-x-0 top-[calc(100%+8px)] z-[160] border px-3 py-4 shadow-2xl" style={{ backgroundColor: ctx.panelTheme.panel, borderColor: ctx.panelTheme.border }}>
            <CoverGridWidthControl start={range.start} end={range.end} onChange={(start, end) => applyColumns(Math.max(1, end - start + 1))} compact />
          </div>
        ) : null}
      </div>
      ...
      <button type="button" onClick={() => setDarkOpen((prev) => !prev)} className="mt-3 flex w-full items-center justify-between border-b px-0 py-2 text-left text-sm transition" style={{ borderColor: darkOpen ? "#2F8EEF" : ctx.panelTheme.border, color: darkOpen ? ctx.panelTheme.text : ctx.panelTheme.muted }}>
