...
function renderFlatTextarea(label: string, value: string, onChange: (value: string) => void, placeholder?: string) {
  return (
    <label className="block text-[11px] font-semibold uppercase tracking-[0.15em] text-[color:var(--bp-muted)]">
      <div className="min-h-[32px] leading-4">{label}</div>
      <div className="mt-2 border-b border-[color:var(--bp-stroke)] pb-1">
        <textarea
          ...
          placeholder={placeholder}
          rows={3}
          className="w-full resize-none appearance-none rounded-none border-0 bg-transparent p-0 text-base font-normal normal-case tracking-normal shadow-none outline-none ring-0 placeholder:text-[color:var(--bp-muted)] focus:border-0 focus:shadow-none focus:outline-none focus:ring-0"
          style={{ border: 0, borderRadius: 0, backgroundColor: "transparent", boxShadow: "none", appearance: "none" }}
        />
      </div>
    </label>
...
function LocationListEditor(ctx: CrmPanelCtx, updateData: (patch: Record<string, unknown>) => void) {
  const mode = (ctx.block.data.mode as string) ?? "all";
  const selected = new Set<number>(Array.isArray(ctx.block.data.ids) ? (ctx.block.data.ids as number[]) : []);

  return (
    <div className="space-y-5">
      {renderFlatSelect("Список филиалов", mode, (value) => updateData({ mode: value }), [
        { value: "all", label: "Все" },
...
function LocationCardEditor({
  ...
  onUpdate?: (location: LocationItem) => void;
}) {
  ...
  const applyLocationUpdate = (patch: Partial<LocationItem>) => {
    onUpdate?.({
      ...location,
      ...patch,
      photoUrls: patch.photoUrls ?? location.photoUrls,
      photoItems: patch.photoItems ?? location.photoItems,
    });
  };
  ...
  const saveLocation = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(payload?.error?.message ?? "Не удалось сохранить филиал.");
        return;
      }
      const updated = payload?.data;
      applyLocationUpdate({
        name: String(updated?.name ?? name),
        address: String(updated?.address ?? address),
        description: updated?.description ?? null,
        phone: updated?.phone ?? null,
        geo: updated?.geo ?? location.geo,
      });
      setMessage("Сохранено");
    } catch {
      setMessage("Не удалось сохранить филиал.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const setCoverPhoto = async (photo: { id: number; url: string }) => {
    setMessage(null);
    const response = await fetch(`/api/v1/crm/locations/${location.id}/media/${photo.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    const existingItems = location.photoItems ?? (location.photoUrls ?? []).map((url, index) => ({
      id: -index - 1,
      url,
      isCover: url === location.coverUrl,
    }));
  ...
  const removeCoverPhoto = async () => {
    const currentCover = (location.photoItems ?? []).find((item) => item.id > 0 && item.url === location.coverUrl);
    if (currentCover) {
      const response = await fetch(`/api/v1/crm/locations/${location.id}/media/${currentCover.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    applyLocationUpdate({
      coverUrl: null,
      photoItems: (location.photoItems ?? []).map((item) => ({ ...item, isCover: false })),
    });
  ...
  const deletePhoto = async (photo: { id: number; url: string }) => {
    ...
    setMessage(null);
    try {
      const response = await fetch(`/api/v1/crm/locations/${location.id}/media/${photo.id}`, { method: "DELETE" });
      if (!response.ok) {
        setMessage("Не удалось удалить фотографию.");
        return;
      }
      const nextItems = (location.photoItems ?? []).filter((item) => item.id !== photo.id);
      const wasCover = location.coverUrl === photo.url;
      applyLocationUpdate({
        coverUrl: wasCover ? null : location.coverUrl,
        photoUrls: nextItems.map((item) => item.url),
        photoItems: nextItems,
      });
  ...
  const uploadPhoto = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/crm/locations/${location.id}/media`, {
        method: "POST",
        body: formData,
      });
      const payload = await response.json().catch(() => null);
  ...
  const photos = location.photoItems ?? (location.photoUrls ?? []).map((url, index) => ({
    id: -index - 1,
    url,
    isCover: url === location.coverUrl,
  }));
  ...
  return (
    <div className="space-y-4 border-t border-[color:var(--bp-stroke)] px-4 pb-4 pt-3">
      <div className="grid gap-4">
        {renderFlatTextInput("Название", name, setName)}
        {renderFlatTextInput("Адрес", address, setAddress)}
        {renderFlatTextInput("Телефон", phone, setPhone)}
        {renderFlatTextarea("Описание", description, setDescription)}
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 text-xs text-[color:var(--bp-muted)]">
            {location.coverUrl ? "Обложка филиала выбрана" : "Обложка филиала не выбрана"}
          </div>
        </div>
        <input
          ref={fileInputRef}
      ...
      >
        {saving ? "Сохранение..." : "Сохранить филиал"}
      </button>
      {pendingDeletePhoto && typeof document !== "undefined"
        ? createPortal(
            ...
            >
              ...
              >
                <div className="text-base font-semibold">Удалить изображение?</div>
...
function LocationCardsEditor({ ctx }: { ctx: CrmPanelCtx }) {
  const [expandedId, setExpandedId] = useState<number | null>(null);

  return (
    <div className="space-y-3">
      {renderSectionTitle("Карточки филиалов")}
      <div className="space-y-3">
        {ctx.locations.map((location) => {
          ...
          return (
            ...
            >
              <button
                type="button"
                onClick={() => setExpandedId((prev) => (prev === location.id ? null : location.id))}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-[color:var(--bp-muted)]">{location.address || "Без адреса"}</div>
                </div>
                <span className="text-sm text-[color:var(--bp-muted)]">{expanded ? "\u25B4" : "\u25BE"}</span>
              </button>
              {expanded ? <LocationCardEditor location={location} onUpdate={ctx.updateLocationItem} /> : null}
            </div>
          );
        })}
        {ctx.locations.length === 0 ? (
          <div className="text-sm text-[color:var(--bp-muted)]">В CRM пока нет филиалов.</div>
        ) : null}
      </div>
    </div>
  );
...
export function LC001ContentPanel(ctx: CrmPanelCtx) {
  ...
  return (
    <div className="space-y-8 px-1 pb-8 pt-1" onClick={(event) => event.stopPropagation()}>
      {inSection("text") && (
        <div className="space-y-5">
          {renderFlatTextInput("Заголовок", String(block.data.title ?? ""), (value) => updateData({ title: value }), "Филиалы")}
          {renderFlatTextInput(
            "Описание",
      ...
      {inSection("catalog") && (
        <div className="space-y-5">
          {LocationListEditor(ctx, updateData)}
          <LocationCardsEditor ctx={ctx} />
        </div>
      )}
    </div>
  );
