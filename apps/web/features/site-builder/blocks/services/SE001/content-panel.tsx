...
function ServiceCardEditor({
  ...
  const [categoryId, setCategoryId] = useState(service.categoryId != null ? String(service.categoryId) : "");
  const [duration, setDuration] = useState(String(service.baseDurationMin));
  ...
  useEffect(() => {
    ...
    setCategoryId(service.categoryId != null ? String(service.categoryId) : "");
    setDuration(String(service.baseDurationMin));
    setPrice(String(service.basePrice));
  }, [service]);

  const applyServiceUpdate = (patch: Partial<ServiceItem>) => {
    onUpdate?.({
      ...service,
      ...patch,
      photoUrls: patch.photoUrls ?? service.photoUrls,
      photoItems: patch.photoItems ?? service.photoItems,
    });
  };
    ...
    try {
      const response = await fetch(`/api/v1/crm/services/${service.id}`, {
        ...
        body: JSON.stringify({
          ...
          baseDurationMin: Number(duration),
          basePrice: price,
        }),
      });
      const payload = await response.json().catch(() => null);
      if (!response.ok) {
        setMessage(payload?.error?.message ?? "Не удалось сохранить услугу.");
        return;
      }
      const updated = payload?.data;
      const nextCategory = updated?.category ?? null;
      applyServiceUpdate({
        name: String(updated?.name ?? name),
        categoryId: nextCategory?.id ?? null,
        categoryName: nextCategory?.name ?? null,
        baseDurationMin: Number(updated?.baseDurationMin ?? duration),
        basePrice: Number(updated?.basePrice ?? price),
      });
  ...
  const setCoverPhoto = async (photo: { id: number; url: string }) => {
    ...
    const existingItems = service.photoItems ?? service.photoUrls.map((url, index) => ({
      id: -index - 1,
      url,
      isCover: url === service.coverUrl,
    }));
  ...
  const uploadPhoto = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/crm/services/${service.id}/media`, {
        method: "POST",
        body: formData,
      });
      const payload = await response.json().catch(() => null);
  ...
  const photos = service.photoItems ?? service.photoUrls.map((url, index) => ({
    id: -index - 1,
    url,
    isCover: url === service.coverUrl,
  }));
    ...
    <div className="space-y-4 border-t border-[color:var(--bp-stroke)] px-4 pb-4 pt-3">
      <div className="grid gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {renderFlatTextInput("Длительность, мин", duration, setDuration)}
          {renderFlatTextInput("Стоимость", price, setPrice)}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-3">
          <div className="h-20 w-28 overflow-hidden rounded-md bg-[color:var(--bp-surface)]">
            {service.coverUrl ? (
              <UnoptimizedImage src={service.coverUrl} alt="" className="h-full w-full object-cover" />
            ) : (
          ...
          <div className="min-w-0 text-xs text-[color:var(--bp-muted)]">
            {service.coverUrl ? "Обложка услуги выбрана" : "Обложка услуги не выбрана"}
          </div>
        </div>
        <input
          ref={fileInputRef}
...
function ServiceCardsEditor({ ctx }: { ctx: CrmPanelCtx }) {
  const [expandedId, setExpandedId] = useState<number | null>(null);

  return (
    <div className="space-y-3">
      {renderSectionTitle("Карточки услуг")}
      <div className="space-y-3">
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-[color:var(--bp-muted)]">
                    {service.categoryName || "Без категории"} · {service.baseDurationMin} мин · {service.basePrice} ₽
                  </div>
                </div>
              ...
              {expanded ? (
                <ServiceCardEditor
                  service={service}
                  categories={ctx.serviceCategories}
                  onUpdate={ctx.updateServiceItem}
                />
              ) : null}
            </div>
          );
        ...
        {ctx.services.length === 0 ? (
          <div className="text-sm text-[color:var(--bp-muted)]">В CRM пока нет услуг.</div>
        ) : null}
      </div>
    </div>
  );
