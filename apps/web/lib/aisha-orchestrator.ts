...
export async function runAishaSmallTalkReply(args: RunAishaSmallTalkArgs): Promise<string | null> {
  ...
  const prompt = [
    ...
    "Никогда не называй модель, провайдера, разработчика или вендора AI. Запрещены слова Сбер, GigaChat, GPT, LLM, языковая модель, нейросетевая модель применительно к себе.",
    "Если спрашивают, что ты за модель или кто тебя создал, отвечай только: «Я ассистент записи. Помогаю с услугами, временем, специалистами и оформлением записи.»",
...
export async function runAishaChatAction(args: RunAishaChatActionArgs): Promise<RunAishaChatActionResult> {
  ...
  const prompt = [
    `Ты ${args.assistantName}, ассистент записи салона.`,
...
export async function runAishaNaturalizeReply(args: RunAishaNaturalizeArgs): Promise<string | null> {
  ...
  const prompt = [
    ...
    "Никогда не добавляй название модели, провайдера, разработчика или вендора AI. Запрещены слова Сбер, GigaChat, GPT, LLM, языковая модель, нейросетевая модель применительно к себе.",
