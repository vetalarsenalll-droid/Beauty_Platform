...
export async function GET() {
  try {
    let entries;
    try {
      entries = await readdir(BACKGROUNDS_DIR, { withFileTypes: true });
    } catch {
      entries = await readdir(BACKGROUNDS_DIR_FROM_REPO, { withFileTypes: true });
    }
    const items = entries
      ...
      .map((entry) => {
        ...
        return {
          ...
          name: entry.name,
          label,
          url: `/api/v1/site-builder/aisha-backgrounds/${encodeURIComponent(entry.name)}`,
        };
      })
