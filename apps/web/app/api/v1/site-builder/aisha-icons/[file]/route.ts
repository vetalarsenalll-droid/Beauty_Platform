...
export async function GET(_request: Request, { params }: Params) {
  ...
  const baseDir = path.isAbsolute(ICONS_DIR) ? ICONS_DIR : path.resolve(ICONS_DIR);
  const repoBaseDir = path.isAbsolute(ICONS_DIR_FROM_REPO) ? ICONS_DIR_FROM_REPO : path.resolve(ICONS_DIR_FROM_REPO);
