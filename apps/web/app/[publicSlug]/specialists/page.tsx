...
export async function generateMetadata({ params }: Pick<PageProps, "params">) {
  ...
  return generatePublicPageMetadata(resolvedParams.publicSlug ?? "", "specialists");
}

export default async function PublicSpecialistsPage({ params, searchParams }: PageProps) {
  const resolvedParams = await params;
