...
export async function generateMetadata({ params }: Pick<PageProps, "params">) {
  ...
  return generatePublicPageMetadata(resolvedParams.publicSlug ?? "", "aisha");
}

export default async function PublicAssistantPage({ params, searchParams }: PageProps) {
  const resolvedParams = await params;
  ...
  const aishaWidgetConfig = resolveAishaWidgetConfig(data.draft, data.draft.theme.mode);
