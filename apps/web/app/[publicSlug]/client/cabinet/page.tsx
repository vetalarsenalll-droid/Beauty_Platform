...
export async function generateMetadata({ params }: Pick<PageProps, "params">) {
  ...
  return generatePublicPageMetadata(resolvedParams.publicSlug ?? "", "clientCabinet");
}

export default async function PublicClientCabinetPage({ params, searchParams }: PageProps) {
  const resolvedParams = await params;
  ...
  return renderPublicPageShell({
    ...
    publicSlug,
    searchParams,
    accountLinkOverride: `/c?account=${data.account.slug}`,
