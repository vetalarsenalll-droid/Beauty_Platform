    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight">Внутренний ledger AI, ₽</h1>
          <p className="mt-2 text-sm text-[color:var(--bp-muted)]">
            Технические рублевые движения для экономики платформы{accountId ? ` аккаунта #${accountId}` : " всех аккаунтов"}.
          </p>
        </div>
        {accountId ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-sm">
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-[color:var(--bp-stroke)]">
                  ...
                  <td className={row.amountTokens < 0 ? "py-2 pr-3 text-rose-600" : "py-2 pr-3 text-emerald-600"}>
                    {row.amountTokens > 0 ? "+" : ""}{row.amountTokens.toLocaleString("ru-RU")}
                  </td>
              ...
              {!rows.length ? (
                <tr><td colSpan={8} className="py-4 text-[color:var(--bp-muted)]">Движений пока нет.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
