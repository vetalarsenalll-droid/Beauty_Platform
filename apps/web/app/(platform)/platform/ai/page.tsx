...
export default async function PlatformAiPage() {
  ...
  const [usageAll, usageMonth, aiAppointments, pools, accountCount] = await Promise.all([
  ...
  const marginAll = chargedAll - costAll;
  const totalTokens = Number(usageAll._sum.totalTokens ?? 0);
  const activePool = pools.find((pool) => pool.isActive) ?? null;
  const packageTokens = activePool?.packageTokens ?? Number(process.env.GIGACHAT_PACKAGE_TOKENS ?? 0);
  const poolSpentPct = packageTokens > 0 ? Math.min(100, (totalTokens / packageTokens) * 100) : 0;
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight">Пул GigaChat и экономика AI</h1>
          <p className="mt-2 max-w-3xl text-sm text-[color:var(--bp-muted)]">
            Бизнес-аккаунты покупают AI-доступ платформы. Ключи GigaChat и реальный пул токенов остаются только на стороне платформы.
          </p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Metric title="Токены всего" value={int(totalTokens)} hint={`За месяц: ${int(usageMonth._sum.totalTokens ?? 0)}`} />
        ...
        <Metric title="Маржа AI" value={`${money(marginAll)} ₽`} hint={`${accountCount} бизнес-аккаунтов`} />
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <article className="rounded-2xl border border-[color:var(--bp-stroke)] bg-[color:var(--bp-paper)] p-5 shadow-[var(--bp-shadow)] xl:col-span-2">
          <h2 className="text-lg font-semibold">Текущий пул провайдера</h2>
          <div className="mt-4 grid gap-3 text-sm">
            <div className="flex items-center justify-between gap-3">
              <span className="text-[color:var(--bp-muted)]">Активный провайдер</span>
              <span className="font-medium">{activePool ? `${activePool.provider} / ${activePool.model}` : "из .env"}</span>
            </div>
            <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center justify-between gap-3">
              <span className="text-[color:var(--bp-muted)]">Расчётный объём пула</span>
              <span className="font-medium">{int(packageTokens)} токенов</span>
            </div>
            ...
            <div className="text-xs text-[color:var(--bp-muted)]">Израсходовано примерно {money(poolSpentPct, 1)}% от расчётного пула.</div>
          </div>
        </article>
