...
const [collapsed, setCollapsed] = useState(false);
  ...
  const [siteOpen, setSiteOpen] = useState(pathname.startsWith("/crm/site"));
  ...
  useEffect(() => {
    if (pathname.startsWith("/crm/site")) {
      setSiteOpen(true);
    }
  }, [pathname]);
    ...
    >
      ...
      {!isSiteBuilderPage && (
          ...
          <div className="flex flex-col gap-1">
            ...
            {afterStaffItems.map((item) => {
              ...
              if (item.href === "/crm/site") {
                if (effectiveCollapsed) {
                  return (
                    ...
                    >
                      ...
                      <span className="whitespace-nowrap opacity-100 translate-x-0 md:pointer-events-none md:w-0 md:overflow-hidden md:opacity-0 md:-translate-x-1">
                        {item.label}
                      </span>
                    </Link>
                  );
                ...
                return (
                  <div key="site-group" className="mt-1">
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="whitespace-nowrap">{item.label}</span>
                      <span className="ml-auto text-xs text-[color:var(--bp-muted)]">
                        {siteOpen ? "-" : "+"}
                      </span>
                    </button>
                    <div className={`mt-1 flex flex-col gap-1 pl-7 ${siteOpen ? "" : "hidden"}`}>
                      {SITE_ITEMS.map((siteItem) => {
                        ...
                        return (
                          ...
                          >
                            <span className="mr-2 inline-flex h-2 w-2 items-center justify-center rounded-full bg-[color:var(--bp-muted)]/40" />
                            <span className="whitespace-nowrap">{siteItem.label}</span>
                          </Link>
                        );
                      })}
                    </div>
        ...
        <div className="shrink-0 border-t border-[color:var(--bp-stroke)] px-3 py-4">
          <Link
            href="/crm/account"
            className={`mb-3 flex items-center gap-3 rounded-2xl px-2 py-1 transition hover:bg-[color:var(--sidebar-item)] ${
              effectiveCollapsed ? "justify-center" : ""
            }`}
            title={effectiveCollapsed ? "Профиль аккаунта" : undefined}
          ...
          >
            ...
            {!effectiveCollapsed && (
              <div className="min-w-0">
                <div className="truncate text-xs text-[color:var(--bp-muted)]">{userEmail}</div>
                <div className="text-xs text-[color:var(--bp-muted)]">CRM оператор</div>
              </div>
            )}
          </Link>
