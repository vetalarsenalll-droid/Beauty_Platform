...
type SpecialistMediaFormProps = {
  specialistId: number;
  title: string;
  type: "specialist" | "work";
  items: MediaItem[];
};
...
export default function SpecialistMediaForm({
  specialistId,
  title,
  type,
  items,
}: SpecialistMediaFormProps) {
    ...
    for (const item of itemsToUpload) {
      ...
      try {
        ...
        const response = await fetch(
          `/api/v1/crm/specialists/${specialistId}/media`,
          { method: "POST", body: formData }
        );
        if (!response.ok) {
          const body = await response.json().catch(() => null);
          const message =
            body?.error?.message ?? "Не удалось загрузить фото.";
          setPending((prev) =>
            prev.map((entry) =>
              entry.id === item.id
  ...
  const removePhoto = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/crm/specialists/${specialistId}/media/${id}`,
        { method: "DELETE" }
      );
      if (!response.ok) {
        const body = await response.json().catch(() => null);
      ...
      {orderedItems.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {orderedItems.map((item, index) => (
            ...
            >
              <div className="relative aspect-[4/3] bg-[color:var(--bp-panel)]/60">
                <UnoptimizedImage src={item.url} alt="" className="h-full w-full object-cover" />
                {item.isCover ? (
