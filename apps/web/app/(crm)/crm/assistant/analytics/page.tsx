        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {totalPages > 1 ? (
            <>
              {page > 1 ? (
                <Link
                  href={`/crm/assistant/analytics?${buildQueryString({ ...baseParams, page: page - 1 })}`}
                  className="rounded-xl border border-[color:var(--bp-stroke)] px-3 py-2 text-sm transition hover:border-[color:var(--bp-accent)]"
                >
                  Назад
                </Link>
              ...
              {pages.map((p) => (
                <Link
                  key={p}
                  href={`/crm/assistant/analytics?${buildQueryString({ ...baseParams, page: p })}`}
                  className={[
