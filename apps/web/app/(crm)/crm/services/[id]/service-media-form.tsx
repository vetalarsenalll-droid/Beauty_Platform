...
export default function ServiceMediaForm({
  serviceId,
  title,
  type,
  items,
}: ServiceMediaFormProps) {
    ...
    for (const item of itemsToUpload) {
      ...
      try {
        ...
        const response = await fetch(
          `/api/v1/crm/services/${serviceId}/media`,
  ...
  const removePhoto = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/crm/services/${serviceId}/media/${id}`,
        { method: "DELETE" }
      );
      if (!response.ok) {
        const body = await response.json().catch(() => null);
