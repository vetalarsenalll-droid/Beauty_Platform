...
export default function SubscriptionCheckout({
  ...
  return (
    <>
      <article className="rounded-2xl border border-[color:var(--bp-stroke)] bg-[color:var(--bp-paper)] p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold">{name}</h3>
            {description ? <p className="mt-1 text-sm text-[color:var(--bp-muted)]">{description}</p> : null}
          </div>
        ...
        <button
          ...
          className="mt-4 w-full rounded-xl bg-[color:var(--bp-accent)] px-4 py-2 text-sm font-medium text-white"
        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-sm text-[color:var(--bp-muted)]">Оплата подписки</div>
            ...
            <div className="mt-4 grid gap-3">
              ...
              <button type="button" disabled={loadingMethod != null} onClick={() => startCheckout("sbp")} className="rounded-xl bg-[#211052] px-4 py-3 text-sm font-semibold text-white disabled:opacity-60">
