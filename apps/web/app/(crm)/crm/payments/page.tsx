...
const paymentScenarioLabels: Record<string, string> = {
  ...
  crm_test_payment: "Тестовый платёж CRM",
};

const providerStatusLabels: Record<string, string> = {
  NEW: "Создан",
  FORM_SHOWED: "Платёжная форма открыта",
...
export default async function CrmPaymentsPage({ searchParams }: PageProps) {
  ...
  const rawParams = (await Promise.resolve(searchParams ?? {})) as Record<
    string,
  ...
  const paymentStatus = isOption(rawPaymentStatus, paymentStatusOptions)
    ? rawPaymentStatus
    : "";
  const paymentProvider = isOption(rawPaymentProvider, paymentProviderOptions)
    ? rawPaymentProvider
    : "";
  const paymentScenario = isOption(rawPaymentScenario, paymentScenarioOptions)
    ? rawPaymentScenario
    : "";
  ...
  const [accountPaymentConnections, clientPayments] = await Promise.all([
  ...
  const paymentConnections = accountPaymentConnections.map((connection: any) => ({
    ...connection,
    credentialsMasked:
      connection.credentialsMasked &&
      typeof connection.credentialsMasked === "object" &&
      !Array.isArray(connection.credentialsMasked)
    ...
    <div className="flex flex-col gap-6">
      <header>
        ...
        <p className="mt-2 text-sm text-[color:var(--bp-muted)]">
          Настройте провайдера, проверяйте подключение и отслеживайте платежи клиентов.
        </p>
      </header>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[color:var(--bp-muted)]">
              Онлайн-оплаты записей, тестовые платежи и статусы провайдеров.
            </p>
          </div>
          <Link
            href="/crm/payments"
            className="rounded-xl border border-[color:var(--bp-stroke)] px-3 py-2 text-sm hover:border-[color:var(--bp-accent)]"
          >
            Сбросить фильтры
          </Link>
        </div>
        ...
        <form method="get" className="mt-4 grid gap-3 md:grid-cols-4">
          ...
          <label className="text-sm">
            ...
            <select
              ...
              defaultValue={paymentScenario}
              className="w-full rounded-xl border border-[color:var(--bp-stroke)] bg-[color:var(--bp-paper)] px-3 py-2"
            >
              <option value="">Все</option>
              {paymentScenarioOptions.map((scenario) => (
                <option key={scenario} value={scenario}>
                  {paymentScenarioLabels[scenario]}
                </option>
              ))}
            </select>
          </label>
          ...
          <div className="md:col-span-4">
            <button
              type="submit"
              className="rounded-xl bg-[color:var(--bp-ink)] px-4 py-2 text-sm font-medium text-white"
            >
              Применить
            </button>
          </div>
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[980px] text-sm">
            <thead className="text-left text-[color:var(--bp-muted)]">
              <tr>
                <th className="py-2 pr-3">Платёж</th>
            ...
            <tbody>
              {clientPayments.map((payment: any) => {
                ...
                return (
                  <tr
                    key={payment.id}
                    className="border-t border-[color:var(--bp-stroke)] align-top"
                  >
                    <td className="py-3 pr-3">
                      <div className="font-medium">#{payment.id}</div>
                      <div className="mt-1 text-xs text-[color:var(--bp-muted)]">
                        {paymentScenarioLabels[payment.scenario] ?? payment.scenario}
                      </div>
                      {payment.providerRef ? (
                        <div className="mt-1 text-xs text-[color:var(--bp-muted)]">
                    ...
                    <td className="py-3 pr-3">
                      ...
                      <div className="mt-1 text-xs text-[color:var(--bp-muted)]">
                        {serviceNames || "Услуги не указаны"}
                      </div>
                      <div className="mt-1 text-xs text-[color:var(--bp-muted)]">
                        {formatPaymentDate(payment.appointment?.startAt)}
                      </div>
                    </td>
                    ...
                    <td className="py-3 pr-3">
                      <div>
                        {payment.provider
                          ? paymentProviderLabels[payment.provider] ?? payment.provider
                          : "—"}
                      </div>
                      <div className="mt-1 text-xs text-[color:var(--bp-muted)]">
                        {providerStatusLabel(payment.providerStatus ?? lastTransaction?.providerStatus)}
                      </div>
                    </td>
                    <td className="py-3 pr-3">
                      ...
                      <div className="mt-2 text-xs text-[color:var(--bp-muted)]">
                        Оплачен: {formatPaymentDate(payment.paidAt ?? lastTransaction?.paidAt)}
                      </div>
                    </td>
                    <td className="py-3 pr-3">
              ...
              {!clientPayments.length ? (
                <tr>
                  <td colSpan={8} className="py-5 text-[color:var(--bp-muted)]">
                    Платежей по выбранным фильтрам нет.
                  </td>
                </tr>
              ) : null}
            </tbody>
