...
(async ()=>{
  const email = `autobook_${Date.now()}@example.com`;
  ...
  const updated = await prisma.appointment.findUnique({ where:{id: appt.id}, select:{status:true} });
  console.log(`APPOINTMENT #${appt.id} STATUS: ${updated?.status}`);
