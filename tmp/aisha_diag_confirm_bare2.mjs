...
(async()=>{
 ...
 const reg=await fetch(`${base}/api/v1/auth/client/register`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,password,accountSlug,firstName:'Case',lastName:'Diag'})});
 ...
 let threadId=0;
 for (const t of ['какая у меня ближайшая запись','можешь отменить ее?','подтверждаю']){
   const o=await chat(cookie,threadId,t); if(!threadId) threadId=o.threadId;
   const act=await prisma.aiAction.findFirst({where:{threadId},orderBy:{id:'desc'},select:{payload:true}});
   const p=act?.payload||{};
   console.log('>',t); console.log('<',String(o.reply).replace(/\n/g,' '));
   console.log('intent=',p.intent,'route=',p.route,'pending=',p.pendingClientActionType,'confirmPending=',p.confirmPendingClientAction,'msgForRouting=',p.messageForRouting);
 }
