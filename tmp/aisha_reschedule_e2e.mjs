...
(async () => {
  const email = `reschedule_${Date.now()}@example.com`;
  const password = 'Passw0rd!123';

  const regRes = await fetch(`${base}/api/v1/auth/client/register`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ email, password, accountSlug, firstName: 'Flow', lastName: 'Mover' }),
  });
  ...
  const endAt = new Date(startAt.getTime() + service.baseDurationMin * 60 * 1000);
  ...
  const appt = await prisma.appointment.create({
    data: {
      ...
      endAt,
      status: 'NEW',
      priceTotal: service.basePrice,
      durationTotalMin: service.baseDurationMin,
      source: 'CHAT_TEST',
      services: { create: [{ serviceId: service.id, price: service.basePrice, durationMin: service.baseDurationMin }] },
    },
    select: { id: true },
  });
  ...
  const turns = [
    ...
    'перенеси ее на 2026-04-10 15:00',
    'подтверждаю',
    'какая у меня ближайшая запись',
  ];
  ...
  const updated = await prisma.appointment.findUnique({ where: { id: appt.id }, select: { status: true, startAt: true } });
  console.log(`APPOINTMENT #${appt.id} => ${updated?.status} ${updated?.startAt?.toISOString()}`);
