...
const scenarios = /** @type {Scenario[]} */ ([
  ...
  { name: "Abuse de-escalation", suites: ["core"], steps: [{ send: "привет сучка", expectAny: [/уваж|давайте общаться|чем.*помочь|здравств|корректн/i] }] },
  ...
  { name: "Capabilities intent", suites: ["core"], steps: [{ send: "что умеешь", expectAny: [/запис|услуг|время|запис[ьи]/i] }] },
  {
    ...
    steps: [
      { send: "а что ты можешь?", expectAny: [/запис|услуг|время|контакт|статистик/i] },
        { send: "а какие именно есть?", expectAny: [/доступные услуги|услуги ниже|выберите нужную/i], rejectAny: [/spa|свадебн|макияж|укладк/i] },
    ],
  },
  ...
  { name: "Booking service-first path", suites: ["booking-e2e"], steps: [{ send: "хочу на маникюр завтра", expectAny: [/филиал|время|слот|локац|услуга|стоим|мин/i] }] },
  {
    ...
    suites: ["booking-e2e"],
    steps: [
        { send: "запиши на удаление зуба", expectAny: [/услугу.*не нашла|выберите.*доступных|филиал|локац/i] },
...
function activeScenarios() {
  if (SUITE === "all" || SUITE === "super") return scenarios;
...
async function sendMessage({ message, threadId }) {
  ...
  const uiOptions = normalizeUiOptions(payload?.data?.ui);
  return {
...
async function main() {
  ...
  if (!targetScenarios.length) {
    throw new Error(`No scenarios selected for suite='${SUITE}'. Use one of: all, super, core, booking-e2e, client-actions`);
  }
