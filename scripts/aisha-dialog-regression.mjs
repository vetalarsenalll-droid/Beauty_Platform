...
const scenarios = /** @type {Scenario[]} */ ([
  ...
  },
  {
    ...
    steps: [
      ...
      { send: "это круто", expectAny: [/круто|отличн|рада|здорово|приятно|помощ|обращ|ассистент записи|чем помочь/i], rejectAny: [/согласие|персональн|подтвердите галочкой/i] },
    ],
  },
  ...
  },
  {
    ...
    steps: [
      ...
      { send: "через ассистента", expectAny: [/согласие|персональн|имя и номер|проверьте данные|выберите специалист|доступны специалисты|как завершим запись|ассистент записи|чем помочь/i] },
    ],
  },
