...
assert(/executeAttentionOverview[\s\S]*isAttentionOverviewMessage[\s\S]*generateCrmAgentInsights[\s\S]*deterministic_attention_overview/s.test(orchestratorSource), "CRM agent must answer attention overview prompts with account-specific insights instead of a generic fallback.");
...
assert(/executeEntityEditFromText[\s\S]*services\.draftUpdate[\s\S]*locations\.draftUpdate[\s\S]*promos\.draftUpdate[\s\S]*specialists\.draftUpdate[\s\S]*deterministic_entity_edit/s.test(orchestratorSource), "CRM agent must prepare service, location, promo and specialist updates deterministically from text.");
assert(/selectedIntent = inferEditIntentFromEntityType\(selected\?\.type[\s\S]*parsedSelectedArgs[\s\S]*parsedSelectedArgs && inferredIntent === selectedIntent/s.test(orchestratorSource), "CRM agent must support short edit continuations after selecting an entity card.");
assert(/selectedSpecialistId[\s\S]*specialists\.draftScheduleUpdate[\s\S]*threadState/s.test(orchestratorSource), "CRM agent must continue set-workday action intents from the selected specialist.");
...
assert(/moveThreadGroup[\s\S]*sortOrder[\s\S]*Move group up[\s\S]*Move group down/s.test(read("apps/web/app/(crm)/crm/assistant/crm-assistant-cockpit.tsx")), "CRM assistant thread sidebar must support manual group ordering.");
