...
const specialistDeepDiveCases = [
  {
    ...
    steps: [
      ...
      { send: "Анна Смирнова", ifReply: /специалист|выберите специалиста/i, expectAny: [/проверьте данные|как завершим запись|нет свободных окон|нет доступных окон|выберите другую дату/i] },
