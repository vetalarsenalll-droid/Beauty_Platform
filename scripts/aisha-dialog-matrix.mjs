...
const interruptionCases = [
  ...
  { id: "price", send: "по стоимости сориентируй", expectAny: [/₽|стоим|цена|услуг|дата|календар|выберите дату/i] },
  { id: "smalltalk", send: "че кого?", expectAny: [/помочь|запис|услуг|уточн|ассистент|привет|конкретно|не поняла|не уловила|поясни|пояснить|когда вам удобно|когда тебе удобно|выберите дату|календар|дата|что-то пошло не так/i] },
  { id: "out-of-domain", send: "расскажи анекдот", expectAny: [/запис|услуг|ассистент|помочь|салон|дата|календар|выберите дату/i] },
...
const specialistDeepDiveCases = [
  {
    ...
    steps: [
      ...
      { send: "Pavel Sidorov", ifReply: /специалист|выберите специалиста/i, expectAny: [/проверьте данные|как завершим запись/i] },
      { send: "а какие еще услуги он делает?", expectAny: [/услуг|специалист|выберите услугу|могу показать/i] },
      { send: "другое время", expectAny: [/время|слот|доступн|выберите/i] },
    ],
  },
  {
