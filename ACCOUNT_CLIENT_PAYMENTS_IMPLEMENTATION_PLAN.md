...
- [ ] Пройти реальный тестовый сценарий ЮKassa на тестовом магазине владельца аккаунта. Отложено до появления тестовых реквизитов ЮKassa.
...
- [x] Добавить отдельный пользовательский сценарий СБП через `GetQr` с QR-экраном и fallback на hosted-форму банка.
...
- [ ] Проверить SberPay/СБП по доступности в кабинете. Отложено до появления тестовых реквизитов Сбера.
...
- [x] Реализовать callback.
- [x] Реализовать возврат.
- [ ] Проверить СБП по доступности в кабинете. Отложено до появления тестовых реквизитов Альфа-Банка.
...
- [ ] Добавить оплату сертификатов. Отложено: сейчас в CRM нет сущности сертификата, сценария покупки, учета номинала, срока действия и использования.
...
### 2026-06-08 - реализован отдельный сценарий СБП T-Банка в онлайн-записи

Статус: завершено в коде

Что сделано:

- T-Банк после `Init` запрашивает `GetQr` в форматах `IMAGE` и `PAYLOAD`.
- `paymentUrl` остается ссылкой на hosted-форму банка для оплаты картой.
- СБП-QR и payload сохраняются отдельно в `providerPayload.paymentMethods`.
- Публичный checkout API возвращает `cardPaymentUrl`, `sbpQrSvg` и `sbpPayload`.
- В онлайн-записи после создания платежа показывается окно выбора: `Оплатить через СБП` с QR-кодом и `Оплатить картой` с переходом на страницу банка.
- Добавлена публичная проверка статуса платежа для polling после сканирования QR.
- Товары и сертификаты помечены как будущие этапы до появления соответствующих сущностей.
- Ручные проверки ЮKassa, Сбера и Альфа-Банка отложены до появления тестовых реквизитов.

Измененные файлы:

- `apps/web/lib/account-payments/types.ts`
- `apps/web/lib/account-payments/providers/tbank.ts`
- `apps/web/app/api/v1/public/payments/checkout/route.ts`
- `apps/web/app/api/v1/public/payments/intents/[id]/status/route.ts`
...
- Ручной тест T-Банка: создать оплату записи, проверить появление QR СБП, кнопку оплаты картой и автоматический переход на success после оплаты.
...
### 2026-06-08 - исправлено зависание онлайн-записи после возврата из оплаты

Статус: завершено

Проблема:

- При переходе на страницу оплаты запись создавалась до подтверждения платежа со статусом `NEW`.
- Если клиент нажимал `Назад` в браузере и не оплачивал, слот мог оставаться занятым активной неоплаченной записью.
- Дополнительно браузер возвращал страницу онлайн-записи из bfcache, из-за чего карточки локаций могли стать некликабельными до ручной перезагрузки.

Что сделано:

- При включенной настройке `Подтверждать запись только после оплаты` записи, созданные для онлайн-оплаты, переводятся во временный неактивный статус `CANCELLED` сразу после создания платежного намерения.
- Такие записи не блокируют расписание и не мешают повторно выбрать то же время, если клиент ушел со страницы оплаты без платежа.
- После успешной оплаты `applyAccountPaymentState` подтверждает временные записи из `metadata.appointmentIds`, переводя их из `CANCELLED` или `NEW` в `CONFIRMED`.
- В `PaymentIntent.metadata` добавлен флаг `appointmentPendingPayment`, чтобы подтверждать из `CANCELLED` только записи, специально созданные как ожидающие оплату.
- При возврате с банковской страницы через кнопку браузера `Назад` онлайн-запись принудительно перезагружается и очищает сохраненное состояние, чтобы клиент попадал в чистый первый шаг.
...
- Выбрать слот, перейти на оплату картой или СБП, нажать `Назад` в браузере: онлайн-запись должна открыться с первого шага, карточки локаций должны быть кликабельны.
- После возврата без оплаты ранее выбранный слот должен снова быть доступен, если его не занял другой клиент.
- После успешной оплаты запись должна появиться в CRM как подтвержденная.

Блокеры:

- Нет.

### 2026-06-08 - улучшена страница успешной оплаты и возврат из банка

Статус: завершено

Что сделано:

- При переходе в банк из онлайн-записи ставится marker возврата, поэтому кнопка браузера `Назад` восстанавливает выбранные услугу, дату, время и сводку, а не открывает запись с начала.
- Добавлена обработка `pageshow`: если браузер вернул онлайн-запись из back/forward cache после перехода в банк, страница один раз перезагружается и поднимает сохраненное состояние без ручного обновления.
- Исправлена защита восстановления: флаг `restoredFromStorageRef` теперь ставится только после реального восстановления, а не при первом чистом входе.
- Уточнено поведение возврата из банка: при кнопке `Назад` онлайн-запись полностью сбрасывается на первый шаг, сохраненное состояние удаляется, выбранное время, активный hold, контакты, согласия и idempotency очищаются.
- Страница `/payment/success` переписана на нормальный русский текст без технического `CONFIRMED`.
- Банковский статус показывается человекочитаемо: например, `Платеж подтвержден банком`.
- Страница успешной оплаты получила более аккуратную карточку с суммой, статусом, записью и кнопкой возврата на сайт.
- QR СБП в модалке онлайн-записи теперь рендерится как inline SVG с `crispEdges`, а не как масштабированная `img`-картинка через `data:`. Это убирает визуальные белые контуры/артефакты вокруг модулей QR.
...
- Ручной тест: после перехода на оплату картой нажать `Назад` в браузере и проверить восстановление сводки записи; после успешной оплаты проверить новую success-страницу.
